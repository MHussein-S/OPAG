OPGmle12<-function(data, starts, method="BFGS"){
  library(AdequacyModel)
  den=function(par,x){shape=par[3];rate=par[4]; dgamma(x,shape=shape,rate=rate)}
  cum=function(par,x){shape=par[3];rate=par[4]; pgamma(x,shape=shape,rate=rate)}
  pdf0<-function(par,x){
    f0=den(par,x)
    c0=cum(par,x)
    a=par[1]
